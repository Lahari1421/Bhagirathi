...
/** shadcn/ui Card primitive (new-york style), themed to the Bhagirathi system. */
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'rounded-3xl border border-ink/10 bg-white text-ink shadow-soft',
        className,
      )}
      {...props}
    />
...
const CardDescription = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('text-sm leading-relaxed text-ink-500', className)} {...props} />
  ),
);
